...
const createUser = async (user_key, device, room_id, ip, member = null, regUser_id = null) => {
    ...
    if (deviceUser && ipUser) {
        canMsg = deviceUser.server_can_public_chat && ipUser.server_can_public_chat;
        canPrivateMsg = deviceUser.server_can_private_chat && ipUser.server_can_private_chat;
        canMic = deviceUser.server_can_use_mic && ipUser.server_can_use_mic;
        canCam = deviceUser.server_can_use_camera && ipUser.server_can_use_camera;
    }
...
const isUserInAnyRoom = (device) => {
    for (const roomId in global.app_users) {
        if (Object.hasOwnProperty.call(global.app_users, roomId)) {
            ...
            if (users.some((user) => user.hasOwnProperty(device))) {
                return true;
            }
        }
    }
...
const isBanned = async (device, key, ip, room) => {
...
const isBannedFromServer = async (device, level) => {
    const query = level
        ? { $or: [{ device: device, type: enums.banTypes.server, level: enums.banTypes.server }] }
...
const isBannedByIp = async (ip, level) => {
    const query = level
        ? { ip: ip, type: enums.banTypes.ip, level: enums.banTypes.server }
